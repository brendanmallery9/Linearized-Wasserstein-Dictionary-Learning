...
Usage example:
nohup python -u hsi/pipeline/corruption_sweep_wrapper.py \
    --script hsi/pipeline/clustering_eval.py \
...
def main():
    ...
    if args.script is None:
        args.script = str(Path(__file__).resolve().parent /
                          "clustering_eval.py")
    else:
        script_path = Path(args.script)
        if not script_path.is_absolute() and not script_path.exists():
            moved_script = Path(__file__).resolve().parent / args.script
    ...
    # Run visualizer
    viz_script = Path(__file__).resolve().parent / "visualize_sweep_tables.py"
    output_stem = Path(args.output).stem
    viz_dir = str(Path(args.output).parent / output_stem)
    print(f"\n{'=' * 70}")
    print(f"Running visualizer -> {viz_dir}/")
    print(f"{'=' * 70}")
    viz_result = subprocess.run([sys.executable, str(viz_script), viz_dir, args.output])
    if viz_result.returncode != 0:
        print(f"[WARN] Visualizer exited with code {viz_result.returncode}")
